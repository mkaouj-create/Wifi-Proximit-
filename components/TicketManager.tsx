      ...
      try {
        ...
        const rows = dataLines.map(line => {
          ...
          return { 
            ...
            price: hasHeader ? (idx.price !== -1 ? parseInt(p[idx.price]?.replace(/\D/g, '')) || 0 : 0) : (parseInt(p[3]?.replace(/\D/g, '')) || 0)
          };
        }).filter(r => r.username && r.username.length > 0);

        if (rows.length === 0) throw new Error("Aucun ticket valide extrait du fichier.");
          ...
          <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl w-full max-w-sm rounded-[3rem] p-10 shadow-2xl animate-in zoom-in duration-300 border border-white/20 dark:border-gray-700/50">
            ...
            <div className="bg-amber-50 dark:bg-amber-900/10 p-5 rounded-[2rem] mb-8 flex gap-4 border border-amber-100 dark:border-amber-800/50">
              ...
              <div className="space-y-1">
                <p className="text-[10px] text-amber-700 dark:text-amber-500 font-black uppercase tracking-widest">Règles de facturation</p>
            ...
            <div className="space-y-6">
              {isSuper && (
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-gray-400 uppercase ml-3 tracking-widest">Destination du stock</label>
                  <select 
