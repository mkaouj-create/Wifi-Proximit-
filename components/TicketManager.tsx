...
import { Search, FileUp, X, Edit2, Trash2, Loader2, Info } from 'lucide-react';
...
const TicketManager: React.FC<{ user: UserProfile, lang: Language, notify: (type: 'success' | 'error' | 'info', message: string) => void }> = ({ user, lang, notify }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (isSuper && results[1]) {
        setAgencies(results[1]);
        if (results[1].length > 0) setImportTargetAgencyId(results[1][0].id);
      }
    } catch (err) {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (lines.length === 0) throw new Error("Format CSV invalide");
        ...
        notify('success', `${res.success} tickets importés. Coût: ${res.cost.toFixed(2)} cr.`);
        setShowImport(false);
        loadData();
      } catch (err: any) {
        notify('error', err.message || "Erreur lors du traitement du CSV");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black dark:text-white uppercase tracking-tight">{t.inventory}</h2>
          <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{filtered.length} tickets</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-3xl overflow-hidden border dark:border-gray-700 shadow-sm overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y dark:divide-gray-700">
            {filtered.slice(0, 50).map(tk => (
              <tr key={tk.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
        ...
        {filtered.length === 0 && !loading && (
          <div className="py-20 text-center text-gray-400 font-bold uppercase text-[10px] tracking-widest">Aucun ticket</div>
        )}
      </div>

      {showImport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in duration-300">
            <h3 className="text-xl font-black uppercase mb-6 tracking-tight">{t.importCsv}</h3>
            <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-2xl mb-6 flex gap-3 border border-amber-100 dark:border-amber-800">
              ...
              <div className="space-y-1">
                <p className="text-[10px] text-amber-600 font-black uppercase tracking-tight">Tarification</p>
            ...
            <div className="space-y-4">
              {isSuper && (
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-gray-400 uppercase ml-2 tracking-widest">Agence Cible</label>
                  <select 
                    className="w-full p-4 bg-gray-50 dark:bg-gray-900 rounded-2xl outline-none font-bold border border-transparent focus:border-primary-500/30 transition-all"
              ...
              <label className="w-full flex flex-col items-center justify-center p-10 border-4 border-dashed rounded-3xl border-gray-100 dark:border-gray-700 hover:border-primary-500 hover:bg-primary-50/10 cursor-pointer transition-all">
                {loading ? <Loader2 size={32} className="animate-spin text-primary-600"/> : <FileUp size={32} className="text-gray-300 mb-3" />}
                <p className="font-black text-[10px] text-gray-400 uppercase tracking-widest">{loading ? "Traitement..." : "Choisir CSV"}</p>
                <input type="file" accept=".csv" onChange={handleImport} className="hidden" disabled={loading} />
              </label>
            </div>
            <button onClick={() => setShowImport(false)} className="w-full mt-6 py-4 text-[10px] font-black uppercase text-gray-400 hover:text-red-500 transition-colors">Annuler</button>
          </div>
        </div>
      )}
    </div>
