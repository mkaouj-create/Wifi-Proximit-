      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] shadow-sm border dark:border-gray-700">
            <div className="flex items-center justify-between mb-8">
                <h3 className="text-lg md:text-xl font-black dark:text-white tracking-tight uppercase">Actions Prioritaires</h3>
        ...
        {/* CONSUMPTION CARD */}
        <div className="bg-primary-600 p-8 md:p-10 rounded-3xl md:rounded-[2.5rem] shadow-2xl shadow-primary-500/30 text-white flex flex-col justify-between relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
                <TrendingUp size={160} />
            </div>
            <div>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-60 mb-1">Système Crédits</p>
                <h4 className="text-2xl md:text-3xl font-black leading-tight uppercase">Consommation</h4>
            </div>
            <div className="mt-8 space-y-5 md:space-y-6">
                ...
                <button onClick={() => onNavigate('settings')} className="w-full py-4 md:py-5 bg-white text-primary-600 rounded-2xl font-black text-[11px] md:text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-all">
                    Historique Crédits
                </button>
            </div>
        </div>
...
const StatCard = ({ title, val, icon, color, trend }: any) => (
  <div className="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-sm border dark:border-gray-700 hover:border-primary-500 transition-all duration-300 group">
    <div className="flex justify-between items-start mb-6">
        <div className={`w-12 h-12 md:w-14 md:h-14 ${color} text-white rounded-xl md:rounded-[1.2rem] flex items-center justify-center shadow-lg shadow-current/20 group-hover:scale-110 transition-transform`}>
    ...
    <p className="text-xl md:text-2xl font-black dark:text-white truncate tracking-tight uppercase leading-none">{val}</p>
  </div>
);

const Shortcut = ({ onClick, title, icon, desc, color }: any) => (
  <button onClick={onClick} className="flex items-center gap-4 md:gap-5 p-4 md:p-5 bg-gray-50/50 dark:bg-gray-700/30 rounded-2xl md:rounded-3xl hover:bg-white dark:hover:bg-gray-700 border border-transparent hover:border-gray-200 dark:hover:border-gray-600 transition-all text-left group">
    <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:rotate-6 ${color}`}>
    ...
    <ChevronRight className="text-gray-300 group-hover:text-primary-500 group-hover:translate-x-1 transition-all shrink-0" size={18} />
  </button>
);
